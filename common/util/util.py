...
class CommonUtil(object):
    ...
    def getFileSize(file_path):
        ...
        return CommonUtil.formatSize(size)
    ...
    @staticmethod
    def decodeTriggleConds(triggle_cond_in_text):
    ...
    @staticmethod
    def decodeTableStageInfo(stage_info):
    ...
    @staticmethod
    def encodeTableStage(stage_info_list):
        stp_dict = {}
    ...
    @staticmethod
    def objectList2Str(object_list):
        ret_str = "["
    ...
    def insertContent2File(content, file_path):
        ...
        with open(tmp_path, "w") as tf:
            ...
            with open(file_path) as f:
                for line in f:
                    tf.write(line)
        CommonUtil.removeLocalFile(file_path)
    ...
    def convertCSV2PipeDelimited(data_file_path):
        if os.path.getsize(data_file_path) == 0:
            return
        new_data_file_path = CommonUtil.rreplace(data_file_path, ".csv", ".txt", 1)
    ...
    def getESType(type_name):
        if type_name == "array":
            ...
            return "float"
        elif type_name == "fractional":
            pass
        elif type_name == "integer":
            return "integer"
        elif type_name == "integral":
            pass
        elif type_name == "long":
            return "long"
        elif type_name == "map":
...
class HiveUtil(object):
    ...
    def exportTable(self, db_name, table_name, is_full, export_dir_uri, compress=False):
        ...
        # First export data
        export_data_path, export_schema_path = CommonUtil.getExportPath(db_name, table_name, is_full, export_dir_uri, "csv")
        if compress:
	...
	print export_statement	
        ...
        with open(tmp_schema_file, "w") as sf:
            CommonUtil.insertContent2File(schema_content, tmp_schema_file)
        hdfsUtil = HDFSUtil()
...
class CronUtil(object):
    ...
    def _parseField(self, field_str, index):
        field_values = []
        f1 = CommonUtil.splitString(field_str, "/")
        range_field = f1[0]
        stride = 1
        if range_field == "*":
            ...
            vals = CommonUtil.splitString(range_field, ",")
            for v in vals:
