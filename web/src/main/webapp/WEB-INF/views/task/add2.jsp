...
<script>
	...
	var str = '<div class="form-group" id="row_'+ n +'" name="trigger_div">'
		...
		+ '<div class="col-sm-3">' + '<select name="trigger_db" id="trigger_db_'
		+ n
	...
	var str = '<div class="form-group" id="row_export_'+ m +'" name="export_div">'
		+ '<label for="disabledSelect" class="col-sm-2 control-label text-right"></label>'
		+ '<div class="col-sm-3">' + '<select name="export_db" id="export_db_' + m
		+ '" onchange="onchange_export_select(' + m + ')" class="form-control" >' + father_select_export
		+ '</select>' + '</div>' + '<div class="col-sm-3">'
		+ '<select id="export_table_'+m+'" name="export_table" class="form-control">' + son_select_export
		+ '</select>' + '</div>' + '<div class="col-sm-4">' + '<span style="display:block;padding-top:7px">'
		+ '<a style="cursor:pointer;color:blue" onclick="delete_row_export(' + m + ')">删除</a>' + '</span>'
		+ '</div>' + '</div>'
	...
	$("#file").fileupload(
		{
			...
			$("#file_lable").html(
				"<input type='hidden' name='bin_file_uri' id='bin_file_uri' value='" + file_name + "' />"
					+ file_name)
		    }
		})
	...
	<div id="content">
		...
		<form class="form-horizontal" action="mrTask/insertSelective">
			<fieldset id="firstDiv">
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<div class="checkbox-inline">
							<label>
								<input type="radio" name="task_type" value="sql" checked>
								SQL
							</label>
						</div>
						<div class="checkbox-inline">
							<label>
								<input type="radio" name="task_type" value="scala">
								SCALA
							</label>
						</div>
						<div class="checkbox-inline">
							<label>
								<input type="radio" name="task_type" value="python">
								PYTHON
							</label>
						</div>
						<div class="checkbox-inline">
							<label>
								<input type="radio" name="task_type" value="java">
								JAVA
							</label>
						</div>
					</div>
				...
				<div class="form-group">
					<label for="disabledSelect" class="col-sm-2 control-label text-right">参数：</label>
					<div class="col-sm-6">
						<input class="form-control" type="text" placeholder="" id="parameter" name="parameter">
					</div>
				...
				<div class="form-group">
					...
					<section class="col col-8"> <label class="file" id="file_lable">
			...
			<fieldset id="secondDiv">
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<div class="checkbox-inline">
							<label>
								<input type="radio" name="option_mode" value="0" checked>
								手动
							</label>
						</div>
						<div class="checkbox-inline">
							<label>
								<input type="radio" name="option_mode" value="1">
								触发
							</label>
						</div>
						<div class="checkbox-inline">
							<label>
								<input type="radio" name="option_mode" value="2">
								定时
							</label>
						</div>
					</div>
				...
				<div class="form-group" id="task_schedule_div">
					<label for="disabledSelect" class="col-sm-2 control-label text-right">触发时间：</label>
					<div class="col-sm-2" style="position: relative">
						<input style="display: inline; width: 200px" class="form-control" type="text" name="search_create_time_begin" id="search_create_time_begin">
						<span class="" style="display: block; width: 1px; height: 21px; position: absolute; left: 180px; top: 6px; background-color: #A2A2A2;"></span>
						<i class="icon-append fa fa-calendar" style="color: #A2A2A2; position: absolute; top: 10px; left: 190px; display: block"></i>
					</div>
					<label for="disabledSelect" class="col-sm-1 control-label text-right">重复：</label>
					<div class="col-sm-2">
					...
					<label for="disabledSelect" class="col-sm-1 control-label text-right">结束重复：</label>
					<div class="col-sm-2">
						<select id="src_db_type" name="src_db_type" class="form-control">
							<option>重复结束日期</option>
							<option>永不</option>
						</select>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<div class="checkbox-inline">
							<label>
								<input type="radio" name="has_derivative_table" value="0" checked>
								否
							</label>
						</div>
						<div class="checkbox-inline">
							<label>
								<input type="radio" name="has_derivative_table" value="1">
								是
							</label>
						</div>
					</div>
